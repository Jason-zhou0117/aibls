...
class DeepSeekBotService:
    ...
    def _call_api(self, messages: List[Dict[str, str]], temperature: float = 0.65, max_tokens: int = 35,logger:Logger=None) -> Optional[str]:
        ...
        if not self.api_key:
            logger.warning("[ERROR] DeepSeek API Key 未设置！")
            return None
    ...
    async def chat(self, system_prompt: str, user_prompt: str, temperature: float = 0.6,logger:Logger=None) -> Optional[str]:
        ...
        return self._call_api(messages, temperature,30,logger)
    ...
    async def generate_danmaku_reply(
        ...
        # 机器人信息
        bot_info = ""
        # logger.debug(f"生成模板前，机器人用户信息：{bot_user}")
        if bot_user:
            bot_name = bot_user.get("nick_name")
            if bot_name:
    ...
    async def generate_gift_reply(
        ...
        user_prompt = f"""最重要：回复必须 10-20 字，你写完后自己数一下，超过10-20 字重新生成.
