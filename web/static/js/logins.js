...
loginJs.pollStatus = function() {
    ...
    fetch('/login/poll?'+params.toString())
        ...
        .then(data => {
            ...
            document.getElementById('qrcode_msg').innerText = data.text;
            //如果成功则跳转到首页
            if (data.code == 0){
                //如果扫码登录成功
                document.getElementById('qrcode_msg').style.color = "green";
                if (timeout_id != ""){
            ...
            //如果保存信息报错或超时，则提示错误信息并终止轮询。等待刷新二维码
                ...
                document.getElementById('qrcode_msg').style.color = "red";
                if (timeout_id != ""){
                   console.log("清除" + timeout_id)
                   clearTimeout(timeout_id);
                   stop_flag = true;
